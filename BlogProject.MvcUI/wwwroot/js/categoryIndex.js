          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if (categoryListDto.ResultStatus === 0) {
                let tableBody = "";
                $.each(
                  categoryListDto.Categories.$values,
                  function (index, category) {
                    tableBody += `
                                  ...
                                  <td>${convertFirstLetterToUpperCase(
  ...
  $(function () {
    ...
    placeHolderDiv.on("click", "#btnSave", function (event) {
      ...
      $.post(actionUrl, dataToSend).done(function (data) {
        ...
        const newFormBody = $(
          ".modal-body",
          categoryAddAjaxModel.CategoryAddPartial
        );
        ...
        if (isValid) {
          ...
          const newTableRow = `
                              <tr name="${
                                categoryAddAjaxModel.CategoryDto.Category.Id
                                                  ...
                                                  <td>${
                                                    categoryAddAjaxModel
                                                      .CategoryDto.Category.Id
                                                  }</td>
                                                  <td>${
                                                    categoryAddAjaxModel
                                                  ...
                                                  <td>${convertFirstLetterToUpperCase(
                                                    categoryAddAjaxModel.CategoryDto.Category.IsActive.toString()
                                                  )}</td>
                                                  <td>${convertFirstLetterToUpperCase(
                                                    categoryAddAjaxModel.CategoryDto.Category.IsDeleted.toString()
                                                  )}</td>
                                                  <td>${
                                                    categoryAddAjaxModel
                                                      .CategoryDto.Category.Note
                                                  }</td>
                                                  <td>${convertToShortDate(
                                                    categoryAddAjaxModel
                                                      .CategoryDto.Category
          ...
          toastr.success(
            `${categoryAddAjaxModel.CategoryDto.Message}`,
            "Başarılı İşlem!"
          );
        } else {
          let summaryText = "";
    ...
    placeHolderDiv.on("click", "#btnUpdate", function (event) {
      ...
      $.post(actionUrl, dataToSend)
        .done(function (data) {
          ...
          const newFormBody = $(
            ".modal-body",
            categoryUpdateAjaxModel.CategoryUpdatePartial
          );
          placeHolderDiv.find(".modal-body").replaceWith(newFormBody);
          const isValid = newFormBody.find('[name="IsValid"]').val() === "True";
          if (isValid) {
            ...
            const newTableRow = `
                              <tr name="${
                                categoryUpdateAjaxModel.CategoryDto.Category.Id
                                                  ...
                                                  <td>${convertFirstLetterToUpperCase(
                                                    categoryUpdateAjaxModel.CategoryDto.Category.IsActive.toString()
                                                  )}</td>
                                                  <td>${convertFirstLetterToUpperCase(
                                                    categoryUpdateAjaxModel.CategoryDto.Category.IsDeleted.toString()
                                                  )}</td>
                                                  <td>${
                                                    categoryUpdateAjaxModel
                                                      .CategoryDto.Category.Note
            ...
            const categoryTableRow = $(
              `[name="${categoryUpdateAjaxModel.CategoryDto.Category.Id}"]`
            );
            newTableRowObject.hide();
